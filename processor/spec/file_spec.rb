...
describe 'File' do
  ...
  it 'should not allow duplicate files to be created' do
    file = Manifester::Processor::File.new(site, data).create!
    expect(
      Manifester::Processor::File.find(:url, data[:location]).count
    ).to eq 1
  end
  ...
  it 'should be able to find a file by date' do
    expect(
      Manifester::Processor::File.find('timestamp.gte', timestamp).count
    ).to eq 1
  end

  it 'should not be able to find a file by a later date' do
    expect(
      Manifester::Processor::File.find('timestamp.gte', updated_at_timestamp).count
    ).to eq 0
  end
  ...
  it 'should require an update with updated_at attribute update' do
    file = Manifester::Processor::File.new(
      site, data.merge(updated_at: updated_at.to_s)
    )
    file.refresh
    expect(file.requires_update?).to be true
  end
  ...
  it 'should be able to find a file by the updated date' do
    expect(
      Manifester::Processor::File.find(
        'timestamp.gte', updated_at_timestamp
      ).count
    ).to eq 1
  end
  ...
  it 'should not find a file using since timestamp with bad status' do
